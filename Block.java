public class Block {
    private final static int[][] ECcodew = {//EC Codewords Per Block
            {7, 10, 13, 17},
            {10, 16, 22, 28},
            {15, 26, 18, 22},
            {20, 18, 26, 16},
    ...
    private final static int[][] blockInform = {//Block Information
    ...
    public static int[] ECMsg(String msgIn, int lv, int EC) {
        int el1 = blockInform[lv * 4 + EC - 5][0], el2 = blockInform[lv * 4 + EC - 5][2],//blocks
                es1 = blockInform[lv * 4 + EC - 5][1], es2 = blockInform[lv * 4 + EC - 5][3],//numbers
                ecl = ECcodew[lv - 1][EC - 1];//ec words
        int[][] tmoutMsg = new int[el1 + el2][];//tmp: blocks of line of numbers and ec words
        for (int i = 0; i < el1; i++) {
            int[] tm = new int[es1];
            for (int j = 0; j < es1; j++) {//separate into blocks
                tm[j] = Integer.parseInt(msgIn.substring((j + i * es1) * 8, (j + i * es1) * 8 + 8), 2);// tm[j + i * es1]
            }
            tmoutMsg[i] = Encode.ec(tm, ecl);
        }
        for (int i = 0; i < el2; i++) {
            int[] tm = new int[es2];
            for (int j = 0; j < es2; j++) {
                tm[j] = Integer.parseInt(msgIn.substring((el1 * es1 + j + i * es2) * 8, (el1 * es1 + j + i * es2) * 8 + 8), 2);//tm[j + i * es2]
            }
            tmoutMsg[i + el1] = Encode.ec(tm, ecl);
        }
        int[] outMsg = new int[el1 * (es1 + ecl) + el2 * (es2 + ecl)];
        int k = 0;

        for (int i = 0; i < Math.max(es1, es2); i++) {//put together numbers
            for (int j = (i >= es1 && i < es2) ? el1 : 0; j < el1 + el2; j++) {
                outMsg[k++] = tmoutMsg[j][i];
            }
        }
        for (int i = 0; i < ecl; i++) {//put together ec words
            for (int j = 0; j < el1 + el2; j++) {
                if (j < el1)
                    outMsg[k++] = tmoutMsg[j][i + es1];
                else outMsg[k++] = tmoutMsg[j][i + es2];
